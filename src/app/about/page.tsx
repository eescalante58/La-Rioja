...
export default async function AboutPage() {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-larioja-azul overflow-hidden font-montserrat">
      ...
      {/* Maintenance of Mission Section */}
      {maintenance && (
        <section className="py-24 bg-white dark:bg-larioja-azul">
          <div className="container mx-auto px-6">
            <ScrollReveal>
              <div className="text-center max-w-4xl mx-auto mb-16">
                ...
                <p className="text-lg text-gray-500 dark:text-white/60 italic font-medium">
                  {maintenance.description ||
                    "ASPAPREM busca recursos y alianzas para seguir creciendo de manera autosostenible."}
                </p>
              </div>
            </ScrollReveal>
                    ...
                    {Array.isArray(card.items) ? (
                      <ul className="space-y-3">
                        {card.items.map((item: string, i: number) => (
                          ...
                          >
                            <div
                              className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${card.dot || "bg-gray-400"}`}
                            />
                            {item}
                          </li>
                        ))}
